		...
		require("oil").setup({
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function()
					...
					map("g?", oil_actions.show_help.callback, { desc = "Show help" })
					map("<CR>", oil_actions.select.callback, { desc = "Select entry" })
					map("<C-t>", function()
					...
					map("<C-c>", oil_actions.close.callback, { desc = "Close oil" })
					map("R", oil_actions.refresh.callback, { desc = "Refresh" })
					map("-", oil_actions.parent.callback, { desc = "Go to parent dir" })
					map("_", oil_actions.open_cwd.callback, { desc = "Open cwd" })
					map("`", oil_actions.cd.callback, { desc = "CD to dir" })
					map("~", function()
						oil_actions.cd.callback({ scope = "tab" })
					end, { desc = "CD to dir (tab scope)" })
					map("gs", oil_actions.change_sort.callback, { desc = "Change sort" })
					map("gx", oil_actions.open_external.callback, { desc = "Open external" })
					map("H", oil_actions.toggle_hidden.callback, { desc = "Toggle hidden files" })
					map("g\\", oil_actions.toggle_trash.callback, { desc = "Toggle trash" })
				end,
			}),
